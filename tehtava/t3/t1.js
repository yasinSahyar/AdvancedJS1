...
// Select DOM elements using destructuring to clean up code
const { tbody: kohde } = document.querySelector('table');
...
const teeRavintolaLista = async (restaurants) => {
  ...
  // Sort restaurants alphabetically by name using destructuring in the sort method
  restaurants.sort(({ name: nameA }, { name: nameB }) => nameA?.localeCompare(nameB) ?? 0);

  // Loop through restaurants using destructuring
